...
- [纽结: 数学奇幻之旅](https://book.douban.com/subject/37647984/)
  - 截至 2026 年初, 纽结题材的书还是很少的, 尤其是中文作者, 更显可贵~
...
```
GPT 架构仅使用原始的 Transformer 解码器部分.
```

```
BPE 算法的原理是将不在预定义词汇表中的单词分解为更小的子词单元,
甚至单个字符, 从而能够处理词汇表之外的单词.
```
...
```
OpenAI 的 GPT 模型使用的是绝对位置嵌入,
这些嵌入会在训练过程中被优化,
有别于原始 Transformer 模型中的固定或预定义位置编码.
```

```
查询
它代表了模型当前关注或试图理解的项 (比如句子中的一个单词或词元).
查询用于探测输入序列中的其他部分, 以确定对它们的关注程度.

键
在注意力机制中, 输入序列中的每个项 (比如句子中的每个单词)
都有一个对应的键. 这些键用于与查询进行匹配.

值
它表示输入项的实际内容或表示.
一旦模型确定哪些键以及哪些输入部分与查询
(当前关注的项) 最相关, 它就会检索相应的值.
```
...
- [深度学习入门 5](https://book.douban.com/subject/37383541/)
  - 这个系列书籍蛮有态度的:
    一方面算是记录了发展的潮起潮落, 比如现在遇冷的 RNN;
    另一方面确实是平静的书籍 (不焦虑, 不功利).
  - 或许将来, 作者可以重新写一本巨著, 一定支持!

```
但如果跳过强化学习的基础知识部分, 直接进入深度强化学习的章节,
那么你对深度强化学习的理解就只能停留在肤浅的层面.
```

> 此所谓: 平静的书籍

```
由于分层 VAE 增加了潜变量的数量, 因此神经网络的处理量也增加了.
这会导致处理时间延长和参数估计变得困难等问题. 为了解决这些问题,
我们将从观测变量到潜变量的转换简单地替换为添加基于正态分布生成的噪声的处理.
这个思路可以作为推导出扩散模型的起点.
通过逐步添加噪声来 "破坏" 数据的过程称为 "扩散过程".
```

> 可以作为推导出扩散模型的起点.

```
由多个正态分布组成的模型称为高斯混合模型,
有时我们也用 GMM 来称呼它.
```
...
```
GMM 基于类别分布对离散值 z 进行采样,
它的局限性在于潜变量被限定为一个离散值.
与之相比, VAE 基于正态分布采样拥有连续值的向量 z.
由于潜变量是连续值的向量, 因此 VAE 能够进行更多样, 更广泛的表示.
```

```
扩散模型需要反复添加噪声, 如果不缩小数据, 那么方差就会持续增大.
```

```
当扩散过程服从正态分布时, 如果时间间隔足够小,
那么逆扩散过程也将服从正态分布.
```

```
不含时 U-Net 的输入是形状为 (C, H, W) 的图像数据,
其中, C 是输入图像的通道数 (RGB 图像为 3),
H 是图像的高度, W 是图像的宽度. 输出是形状为
(D, H, W) 的张量, 其中 D 是要分类的类别数.
模型对每个像素输出 D 个类别的概率分布.
而在扩散模型中, 输出被设置为 (C, H, W) (D = C),
即输入和输出的通道数都被设置为 C.
```

```
U-Net 的处理过程分为前半部分的 "缩小阶段" 和后半部分的 "扩大阶段".
在前半部分的缩小阶段中, 在卷积层进行处理的同时, 特征图会逐渐缩小.
缩小特征图的层称为 "下采样层". 在后半部分的扩大阶段中,
在卷积层进行特征抽取的同时, 特征图会逐渐扩大, 这与前半部分正好相反.
扩大特征图的层称为 "上采样层". U-Net 的重要特征是跳跃连接.
这是一种在网络的缩小阶段和扩大阶段之间直接传递特征图的机制.
这种跳跃连接使得 U-Net 能够捕捉对象整体的特征,
同时使用更精细的空间位置信息进行处理.
```
