...
- 自旋和统计之间的这种联系 (玻色子和费米子具有完全不同的统计性质)
  ...
  - 因此, 两个相同的费米子 (例如, 两个电子) 不能占据相同的状态. 因为如果
    ...
    我们将得不到任何波函数.
  - 这就是著名的`泡利不相容原理`.
    这不是 (正如你可能已经相信的那样) 一个只适用于电子的奇怪的特定假设,
...
- __时间平移不变性__: 如果哈密顿量是含时的,
  ...
  - 对于无穷小的时间间隔
    ...
    $$.
- 时间平移不变性意味着时间演化与我们考虑的时间间隔无关. 换句话说, 对于选择任意的
  $$ t_1 $$
  和
  $$ t_2 $$,
...
- 玻恩级数可以理解为, 零阶波函数
  ...
  二阶波函数则是先被势`踢`一次后, `传播`到某个位置,
  再次被势`踢`一次, 然后沿着某个新方向`传播`出去; 以此类推.
