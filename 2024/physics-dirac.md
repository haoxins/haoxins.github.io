...
- 自旋磁矩与自旋角动量的方向由同一个矢量
  ...
  - 我们运用一个只依赖于量子理论以及相对论的一般原理的推理, 就能得到
    ...
    这个结论是满意的, 但也有某些种类的基本粒子
    (如光子和某些介子), 从实验上已知其自旋不同于
    $$ \frac{1}{2} \hbar $$,
    这样, 理论与实验之间还存在分歧.
  - 问题的答案隐藏在我们工作的假定之中.
    只有在假定粒子的位置是可观测量的前提下,
...
- 我们现在有一个物理解释的普遍方法, 它与通常的解释类似, 但也有重大差异.
  ...
  - 存在潜在项的结果是, 如果
    $$ K \mid Q_t \rangle $$
    在某个时刻是归一化的, 那么在其他时刻通常并不是归一化的. 因此,
    为了得到概率, 我们需对每个时刻进行分开的归一化.
...
> 入门佳作, 首推**科恩**的量子力学; 而狄拉克体现出来的,
  是真正的**做**物理, `俯视`物理的宗师风范, 非一般书籍可见.
...
```
由于时空是弯曲的, 那么矢量平移的观念就要改变.
原来平坦时空中任何矢量可以平行于自身而移动, 但如果底流形不是平直的,
在一个时空点定义的矢量不能按原来的方式"平行"移动到邻域一个时空点,
因为基底改变了. 为了合理地定义矢量平移,
原有的对时空的微商就要修正为"协变"微商, 也就是增加一个仿射联络项.
```
