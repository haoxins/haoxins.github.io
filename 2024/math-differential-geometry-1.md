...
- 注意, 如果一般曲面的两个主曲率有相同的正负号,
  便有下述几何解释: 这个曲面的外在曲率等于半径为
  $$ 1 / \sqrt{κ_1 κ_2} $$
  的球面的外在曲率.
  - 另一个例子是半径为
    $$ R $$
...
- 如果点
  ...
  - 想象
    ...
    附加正负号的原则取决于球面映射是保持
    $$ δ \widetilde{\mathcal{A}} $$
    的边界走向, 还是反转其边界的走向. 也就是说, 如果
    $$ δ \widetilde{\mathcal{A}} $$
    的边界走向 (从球面外看) 是逆时针方向的, 与原像`相同`, 就取正号;
    如果是顺时针方向的, 与原像`相反`, 就取负号.
...
- 在曲面上点
  ...
  $$ \mathbf{v} $$
  移动时, 法向量是如何变化的
  (即其端点指向哪个方向和移动得有多快). 这时不再要求
  $$ \mathbf{v} $$
  是很短的向量, 而是可以有任意长度. 更准确地说, 它被简单地定义为
  $$ \mathbf{n} $$
  沿着
  $$ \mathbf{v} $$
  移动的负方向导数:
  - $$ \mathcal{S} (\mathbf{v}) ≡ - \nabla_{v} \mathbf{n} $$.
  - 在定义中插入负号是标准做法, 我们很快会看到,
    这是由我们早先定义的法向截面曲率的符号决定的.
...
- 曲率
  $$ \mathcal{K} $$
  正负号的改变导致的拓扑结果就是球面像的折叠.
  当我们越过曲率具有两个不同正负号的区域之间的边界时,
  球面像或者增加, 或者减少符号相反的两层, 从而覆盖
  $$ \mathbb{S}^2 $$
  的次数的代数和 (就是度) 保持不变, 所以全曲率是拓扑不变量.
...
```
我们把这样的模式称为 "无方向流", 但是实际上,
霍普夫把这个概念命名为线元素场.
然而, 我们今后要采用现代术语, 称之为线场, 类比于向量场.
```

- 如你所见, 这些 "流线" 上没有箭头,
  但我们仍然可以给切线配置一个连续变化的角度
  $$ Θ $$.
  如果沿围绕奇点
  $$ s $$
  的简单环路
...
```
虽然我们不能将分数指数的线场解释为无阻塞流的流线,
但是可以将其解释为存在一维障碍的流.
```

- 从这个角度来看, 障碍上的每个点都是向量场的奇点,
  因为当我们穿过障碍时, 向量的方向会不连续地反转.
  然而, 对于上述的无方向线场, 每一个这样的点
  $$ p $$
  (除
  $$ s $$
  外) 都是一个`正则`点.
...
- 至此呈现的 GGB 的三种证明, 都依赖于将
  $$ \mathcal{K} $$
  解释为球面映射的面积扩张系数.
  这就是曲率的一个`外在`概念, 因为它依赖于法向量
  $$ \mathbf{n} $$,
  而曲面的法向量对
  $$ \mathcal{S} $$
  内的居民是不可见和不可知的.
  - 虽然这三个证明在许多不同的方向上为我们提供了奇妙的新见解,
    但是我们还缺少一些东西, 因为我们知道
    $$ \mathcal{K} $$
    实际上也是曲面的`内蕴`性质, 是可以被曲面内的居民感知和测量的.
  - 一般而言, 曲面内的居民可以测量整个曲面的
    $$ \mathcal{K} $$
    值, 通过这些纯粹的局部几何测量,
    他们可以确定自己所在世界的拓扑结构!
...
- 如果我们试图在双曲平面
  $$ \mathbb{H}^2 $$
  中重复欧几里得结构, 那么马上就会遇到麻烦,
  因为已知一个点和经过这个点的一条直线
  $$ L $$,
  我们知道在双曲平面上有`无穷多`条 "平行线"
  穿过每一个与之相邻的点. 我们应该选哪一个呢?!
...
```
在什么情况下, 初始切向量沿曲线 G 的平行移动会保持与 G 相切?
答案是: 只有当 G 是测地线时, 这才会发生.
```

```
现在我们将描述平行移动的第三种外在方法, 并给出平行移动的重要性质.
这些性质在前面看到的两个构作中并没有立即显现出来.
```
...
- 这里有一种看待这个内蕴导数的`外在`方法. 因为内蕴导数
  ...
  - 换句话说, 为了求得
    ...
    然后减去不与曲面相切的部分, 剩下的就是曲面内蕴的部分了.
  - 可以证明内蕴导数
    $$ D_{\mathbf{v}} $$
    和全向量导数
    $$ \nabla_{\mathbf{v}} $$
    一样, 具有线性性质, 并服从莱布尼茨 (乘法) 公式.

> 本书称`协变`导数为`内蕴`导数

```
"协变",
是指它所修饰的物理量在坐标变换中的变换公式与坐标变换公式相同.
相对的概念是 "逆变",
是指它修饰的物理量在坐标变换中的变换公式是坐标变换公式的逆变换.
```
...
- 内蕴导数使得我们对早先关于`测地曲率`的讨论有了新的认识.
  ...
  $$
    \nabla_{\mathbf{v}} \mathbf{v} =
    \mathbf{κ} = \mathbf{κ}_g + \mathbf{κ}_n
  $$.
  - 第一个分量
    $$ \mathbf{κ}_g $$
    是`测地曲率向量`: 它是加速度与
...
- 直观地看,
  ...
  $$ \mathbf{v} $$
  的内蕴转向率:
  - $$ \mathbf{κ}_g = D_{\mathbf{v}} \mathbf{v} $$
- 令
  $$ \mathbf{w}_{\shortparallel} $$
  为
  $$ \mathbf{w} $$
...
- 在
  ...
  因此, `测地线方程`具有如下形式:
  - $$
      \mathbf{κ}_g = 0 \Longleftrightarrow
      D_{\mathbf{v}} \mathbf{v} = 0
    $$
  - 这个结果的另一个表述就是:
    $$ \mathbf{v} $$
    是它沿自身的平行移动, 在
  ...
  - 另外, 如果
    $$ \mathcal{S} $$
    的曲线`不是`测地线, 则当削下它周围的窄带并压平在一个平面上时,
    它仍是弯曲的. 它在平面内的曲率就是
    $$ \mathbf{κ}_g $$!
...
- 一个环路的和乐性就是这个环路内部的全曲率. 如果
  ...
  的曲率:
  - $$
      \mathcal{K} (p) = \lim_{L_p \to p}
      \frac{\mathcal{R}(L_p)}{\mathcal{A}(L_p)} =
...
- 当一般曲面上的一般测地线三角形
  ...
  $$ Δ $$
  平行移动一圈, 回到起点成为
  $$ \mathbf{v}_{\shortparallel} $$
  时, 旋转的角度为和乐性
  $$ \mathcal{R}(Δ) $$.

- 在对霍普夫`旋转定理`的讨论中, 我们就注意到,
  如果质点绕一个欧几里得三角形
  $$ Δ $$
  移动, 那么速度向量的旋转角是
  $$ (φ_1 + φ_2 + φ_3) = 2π $$.
  因此, 和乐性公式量度的是速度向量的净旋转角
  $$ (φ_1 + φ_2 + φ_3) $$
  与欧几里得几何中的
  $$ 2π $$
  相差多少.
  - 在这本书里, 我们已经曾利用另一种方法 (即它的`角盈`
    $$ \mathcal{E} (Δ) $$)
    来量度一个弯曲曲面上的测地线三角形不同于欧几里得平面上三角形的程度.
  - 但事实上, 这两个对曲率在概念上不同的量度在
    $$ Δ $$
    内是相等的!
  - $$ \mathcal{R} (Δ) = \mathcal{E} (Δ) $$.

- 上面的这些讨论都可以很容易地从测地线三角形 (3 边形) 推广到测地线
  $$ m $$
  边形
  $$ P_m $$.
  - 我们发现
    $$ P_m $$
    内曲率的两种看似不同的量度实际上是相等的:
  - $$ \mathcal{R} (P_m) = \mathcal{E} (P_m) $$.
...
- __漂亮定理的几何证明__. 曲面
  ...
  - 进一步有
    $$
      \widetilde{\mathcal{A}} (\widetilde{Δ}) =
      \widetilde{\mathcal{R}} (\widetilde{L})
    $$.
    而球面映射保持平行移动不变, 即
    $$
...
- 当曲面的一个切向量绕
  $$ L $$
  平行移动一圈后, 我们可以看到曲面上的净旋转
  (和乐性) 等于球面上的净旋转:
  - $$
      \mathcal{R} (L) =
      \widetilde{\mathcal{R}} (\widetilde{L})
    $$.
  - 但是, 根据 "至今所知的故事", 球面上的净旋转只是
    $$ \widetilde{L} $$
    在球面上包围的面积. 反过来, 它是由
    $$ L $$
    在曲面
  ...
  - 因为
    ...
    上的, 即它在
    $$ \mathcal{S} $$
    的等距变换下是不变的, 所以我们就证明了高斯 1816 年最初的漂亮定理.
...
- 正如我们定义的那样, 和乐性是一个仅对`闭环`
  ...
  当
  $$ \mathbf{w}_{\shortparallel} $$
  回到平行移动的起点时, 我们可以将它与初始向量
  $$ \mathbf{w}_0 $$
  比较, 看看由此经历的旋转角
  $$ \mathcal{R}(L) $$
  有多大.
  - 霍普夫论证的第一步是将和乐性概念推广到`非闭合`的曲线
    $$ K $$,
    我们显然应该将它定义为沿
    $$ K $$
    平行移动生成的
...
- 我们加入一个基准向量场
  ...
  $$ \mathbf{w}_{\shortparallel} $$
  的
  $$ \angle U \mathbf{w}_{\shortparallel} $$
  的净变化:
  - $$
      \mathcal{R}_U (K) ≡ δ_K
      (\angle U \mathbf{w}_{\shortparallel})
  ...
  - 注意: 因为
    ...
    必不可少. 因此
    $$ \mathcal{R}_U (K) $$
    没有真正的数学意义, 也就是说, 如果两个不同的向量沿
    $$ K $$
    做平行移动, 可知它们之间的夹角保持不变, 因此它们都相对于
    $$ U $$
    旋转了相同的角度.
  - 由此可知,
    $$ \mathcal{R}_U (K) $$
...
- 我们注意到, 这种方法为和乐性提供了一个比以前更好的定义,
  解决了我们在最初的讨论中忽略了的一个潜在问题: 如果
  $$ \mathcal{R} (△) > 2π $$
  会怎样?
  - 如果天真地比较
    $$ \mathbf{w}_{\shortparallel} $$
...
- __正交坐标系中的度量公式__. 在画出一组 (互不相交的)
  ...
  - 地图上很小的水平移动
    ...
    $$ δ X \asymp A δ u $$
    ($$
      δ X \asymp A δ u \Longleftrightarrow
      A \mbox{ 是局部水平扩张系数}
    $$),
  - 地图上很小的竖直移动
    $$ δ v $$
