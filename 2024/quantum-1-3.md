...
- 应用算符函数时, 必须注意算符的顺序. 例如, 若
  ...
  - 反之, 若
    ...
    $$ B $$
    是可对易的, 那么, 很简单
    $$ [A, B] = 0 \Rightarrow e^A e^B = e^B e^A = e^{A + B} $$
  - (此外, 在以
    $$ A $$
    和
    $$ B $$
...
- 算符
  ...
  将态空间
  $$ \mathcal{E} $$
  中的正交归一基矢变换为另一个正交归一基矢.

> 可以基于演化算子应该具有的性质, 反过来得到演化算子的幺正性.
...
## 对易子等于 iℏ 的两个观察算符 Q 和 P 的一些普遍性质
...
- 我们考虑观察算符
  ...
  - 我们定义依赖于实参数
    ...
    为:
  - $$ S(λ) = e^{-iλP / \hbar} $$
  - 这个算符是幺正的. 事实上, 很容易证明下列关系式:
  - $$ S^{\dagger}(λ) = S^{-1}(λ) = S(-λ) $$

### Q 的本征值及本征矢

### q 表象

### p 表象. 观察算符 P 和 Q 之间的对称性
