...
- 每一次递归, 从上到下, 所需要的计算量分析如下:
  ...
  - 第二次, 由于有两个子序列, 而每个子序列的计算量是原先的一半,
    因此总的计算量和第一次相同, 我们可以认为是
    $$ O(N / 2 * 2) = O(N) $$.
  - 第三次, 有了四个子序列, 每一个是原始序列的
    $$ 1/4 $$,
    因此总的计算量和第一次相同.
  - 最后, 到了第
    $$ K = \log N $$
...
- 最坏时间复杂度控制在
  $$ O(N \log N) $$
  量级, 同时还能够保证排序`稳定性`这样一举三得的混合排序算法.
...
- 幂集公理 (Axiom of Power Set):
  - 每一个集合都有其幂集, 即对于任何集合
    ...
    $$ X $$
    的子集.
- 正则公理 (Axiom of Regularity/Axiom of Foundation):
  - 每一个非空集合
    $$ X $$
    总包含着一个元素
    $$ y $$,
...
- 但是处理器内部并没有除法器,
  实际上除法是通过乘法, 加减法和部分查表来完成的,
  一次除法相当于十几次的`乘法`和`加减法`.
- 这也就是说,
