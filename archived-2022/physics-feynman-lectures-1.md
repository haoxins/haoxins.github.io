...
* 量 __a·b__ 称为两个矢量 `a` 和 `b` 的**标积**, 它具有很多重要而有用的性质.
  - 例如, 很容易证明 __a·(b + c) = a·b + a·c__
* 还有, 计算 __a·b__ 有一个不需要计算 `a` 和 `b` 的分量的简单的几何方法:
  - __a·b__ 是 `a` 的长度和 `b` 的长度之积乘以它们之间夹角的余弦.
  - 为什么?
  - 假设我们选一个 `x` 轴沿 `a` 方向的特殊的坐标系, 在这种情况下,
    $$ a_x $$
...
* 因此, 在此特殊的坐标系中, 我们已经证明了 __a·b__ 等于 __a__ 之长乘 __b__
  之长再乘
  $$ \cos{\theta} $$.
