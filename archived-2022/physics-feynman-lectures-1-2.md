...
- 应用电场表示式 (`29.1`), 我们发现
  - $$ S = \frac{q^2 a'^2 \sin ^2 \theta}{16 \pi ε_0 r^2 c^2} $$
    (`32.2`)
  - 就是在
    $$ \theta $$
    方向每平方米所辐射的功率.
...
- 当我们有一个振动系统时, 为了说明公式 (`32.5`) 的结果,
  我们来看一下如果电荷的位移工在作振动, 因而使加速度 `a` 为一
  $$ - ω^2 x_0 e^{iωt} $$
  时, 情况将怎么样.
  - 加速度平方在一周中的平均值
...
- 那么, 当我们取一个固有频率为
  $$ \omega_0 $$
  的正常原子, 并让它以速率 `v` 趋近观察者运动时,
  其频率是否为
  $$ \omega = \omega_{0} / (1 - v/c) $$?
  - 不, 我们知道得很清楚, 由于时间流率的相对论性膨胀,
    运动原子的固有频率
    $$ \omega_1 $$
    与原子静止时量得的不同.
  - 这样一来, 如果真正的固有频率为
...
- 结果表明定义一个矢量
  ...
  $$ 2π / \lambda $$,
  并且指向波的传播方向
  - $$ \mathbf{k} = 2π \mathbf{e}_k / \lambda = k \mathbf{e}_{k} $$.
    (`34.19`)
- 利用这一矢量, 上述波可以写成
